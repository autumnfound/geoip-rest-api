...
public class CityResourceTest {
	...
	public void testCityIPEndpoint() {
		given().when().get("/cities/"+VALID_IPV4_ADDRESS).then().statusCode(200);
		given().when().get("/cities/"+VALID_IPV6_ADDRESS).then().statusCode(200);
	}

	@Test
	public void testCityBadIPEndpoint() {
		// IPv4 tests
		given().when().get("/cities/bad.ip.add.res").then().statusCode(400);
		given().when().get("/cities/300.0.0.0").then().statusCode(400);
		given().when().get("/cities/1.300.0.0").then().statusCode(400);
		given().when().get("/cities/1.0.300.0").then().statusCode(400);
		given().when().get("/cities/1.0.0.300").then().statusCode(400);
		given().when().get("/cities/sample").then().statusCode(400);
		given().when().get("/cities/"+VALID_IPV4_ADDRESS+":8080").then().statusCode(400);
		// seems to be an issue with Google Guava code, only gets detected by MaxMind
		given().when().get("/cities/0.1.1.1").then().statusCode(500);
		...
		given().when().get("/cities/bad:ip:add::res").then().statusCode(400);
		// loopback + unspecified address
		given().when().get("/cities/::").then().statusCode(400);
		given().when().get("/cities/::1").then().statusCode(400);
	}
}
